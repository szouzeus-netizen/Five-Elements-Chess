...
import { newState, playSmall, refineBig, type Element, type State } from './engine';
...
export class WuxingGame extends DurableObject<Env>{
  ...
  async fetch(req:Request){
    ...
    let r:any;
    if(body.action==='reset'){this.state=newState(body.start||'metal'); await this.ctx.storage.delete('past');}
